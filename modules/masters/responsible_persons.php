<?php
/**
 * Responsible Person Master - Mini ERP System
 */

$pageTitle = 'Responsible Person Master';
$currentModule = 'responsible_persons';
...
if (!empty($search)) {
    $whereClause .= " AND (responsible_person_name LIKE ? OR mobile_number LIKE ?)";
    $searchTerm = "%$search%";
    $params[] = $searchTerm;
    $params[] = $searchTerm;
    $types .= "ss";
...
// Count total
$countQuery = "SELECT COUNT(id) as total FROM tbl_responsible_persons WHERE $whereClause";
...
// Fetch records
$query = "SELECT * FROM tbl_responsible_persons WHERE $whereClause ORDER BY responsible_person_name ASC LIMIT ? OFFSET ?";
...
<div class="page-header animate-in">
    <div>
        <h1>Responsible Person Master</h1>
        <p class="page-header-desc">Manage responsible persons for the ERP system.</p>
    </div>
    <?php if (has_permission('master-management', 'create')): ?>
        <div>
            <button type="button" class="btn btn-primary" onclick="openCustomModal('addModal')">
                <i class="fa-solid fa-plus"></i> Add Responsible Person
            </button>
        </div>
    <?php endif; ?>
</div>
            ...
            <tbody>
                ...
                <?php else: ?>
                    <?php foreach ($records as $r): ?>
                        <tr>
                            <td style="font-weight:600;"><?= e($r['responsible_person_name']) ?></td>
                            ...
                            <td style="text-align: right;">
                                <?php if (has_permission('master-management', 'edit')): ?>
                                    <button class="btn btn-sm btn-outline" onclick="openCustomEditModal(<?= $r['id'] ?>, '<?= e(addslashes($r['responsible_person_name'])) ?>', '<?= e(addslashes($r['mobile_number'])) ?>')">
                                    ...
                                    <?php if ($r['status'] === 'active'): ?>
                                        <form method="POST" action="action.php" style="display:inline;">
                                            ...
                                            <input type="hidden" name="master_type" value="responsible_person">
                                            <input type="hidden" name="id" value="<?= $r['id'] ?>">
                                            <input type="hidden" name="new_status" value="inactive">
                                            <input type="hidden" name="return_status" value="<?= e($statusFilter) ?>">
                                            <button type="submit" class="btn btn-sm" style="color:var(--color-danger); border:1px solid rgba(239,68,68,0.2); background:white;" onclick="return confirm('Deactivate this responsible person?')">
                                    ...
                                    <?php else: ?>
                                        <form method="POST" action="action.php" style="display:inline;">
                                            ...
                                            <input type="hidden" name="return_status" value="<?= e($statusFilter) ?>">
                                            <button type="submit" class="btn btn-sm btn-outline" style="color:var(--color-success); border-color:rgba(16,185,129,0.2);" onclick="return confirm('Activate this responsible person?')">
        ...
        <form method="POST" action="action.php">
            ...
            <div class="modal-body">
                <div class="form-group">
                    <label class="form-label">Responsible Person Name <span class="text-danger">*</span></label>
